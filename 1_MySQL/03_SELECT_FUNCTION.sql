...
*/
SELECT length('데이터베이스'), char_length('데이터베이스'), length('database'),
...
SELECT emp_name, email, replace(email,'kh.or.kr','gmail.com') "변경 후 이메일"
FROM employee;
-- 2. 사원명과 주민등록번호(000000-0******)으로 조회
SELECT emp_name, replace(emp_no, substr(emp_no, instr(emp_no,'-') + 2, 6),'******')
FROM employee;
-- rpad
...
*/
SELECT 10 DIV 3, 10 / 3, 10 MOD 3, 10 % 3, mod(10,3);
...
/*
    ...
    TIMESTAMPDIFF(날짜단위, 날짜, 날짜) : 두 날짜 사이의 기간을 날짜단위에 따라 반환
...
SELECT emp_name, hire_date, timestampdiff(day, hire_date,now()) "일수",
	   timestampdiff(month,hire_date,now()) "개월 수",
	   timestampdiff(year,hire_date,now()) "년 수"
FROM employee;
...
SELECT emp_name, year(hire_date) 입사년도, month(hire_date) 입사월, 
day(hire_date) 입사일
FROM employee
...
SELECT now(), date_format(now(),'%Y.%m.%d'), date_format(now(),'%Y.%m.%d.%T');
-- %Y : 년도, %m : 월, %d : 일
-- %T : 시간 전체, %H : 시, %i : 분, %s : 초
...
SELECT emp_name, bonus, (salary + salary * ifnull(bonus,0)) * 12 "보너스 포함 연봉"
FROM employee;
...
SELECT dept_code, emp_name, emp_no,
...
if(job_code='J5', salary * 1.2, salary * 1.05))), 0) 인상된급여
FROM employee
ORDER BY job_code, 인상된급여 desc;
-- ORDER BY 2, 4 desc; 
/*
