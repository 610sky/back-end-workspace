/*
    ...
    (Create추가 Read보기 Update수정 Delete삭제)
...
*/
SELECT emp_name 직원명, salary "직원 월급", salary * 12 as 연봉
FROM employee;
...
-- OR(또는), AND(그리고)
-- 부서코드가 'D6'이거나 급여가 100만원 이상인 사원들의 사원명, 부서코드, 급여 조회
SELECT emp_name, dept_code, salary
FROM employee
WHERE dept_code='D6' OR salary >= 1000000;
...
*/
-- 입사일이 '1990-01-01' ~ '2001-01-01' (모든 컬럼 조회)
SELECT *
FROM employee
...
-- 이름 중에 '하'가 포함된 사원들의 사원명, 주민번호(emp_no), 전화번호 조회
...
-- 3. 연봉(보너스포함X)이 300만원 이상이고 보너스를 받지 않은 사원들의 사번, 사원명, 급여, 보너스 조회
SELECT emp_id, emp_name, salary, bonus
FROM employee
...
-- 6. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사원들의 사번, 사원명, 급여,
-- 보너스 포함 연봉(별칭부여) 조회
-- 보너스 포함 연봉 : (salary + salary * bonus) * 12
SELECT emp_id, emp_name, salary, (salary + salary * bonus) * 12 "true_money"
FROM employee
